...
az_pts = np.linspace(-45, 0, num=45) * np.pi / 180. + np.pi/2
el_pts = np.linspace(-45, 0, num=45) * np.pi / 180.
...
for ia, az in enumerate(az_pts):
    for ie, el in enumerate(el_pts):
        ...
        rays, N_rays = ray_sets.random_disc(2.5, 2000, [x, y, z], nhat)
        ...
        max_consecutive_hits = 5 # indicates a ray may be trapped inside a mesh
        N_bounces = 50
        i_bounce = 0
        print('Tracing rays...')
        ...
        for path in paths:
            ...
            #     continue
            if mesh_id_to_name[surf_id] == 'cryostat_window':
                N_cryo += 1
            if path.incident:
                N_incident += 1
